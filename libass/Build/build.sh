...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/libass.spec
yum-builddep -y ~/rpmbuild/SPECS/libass.spec
rpmbuild -bi ~/rpmbuild/SPECS/libass.spec
rpmbuild -bl ~/rpmbuild/SPECS/libass.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/libass.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/libass.spec
rpmbuild -ba ~/rpmbuild/SPECS/libass.spec
