...
#! /bin/sh
rpmbuild -bi ~/rpmbuild/SPECS/supertuxkart.spec
if [[ 0 -eq 0 ]]
then
    rpmbuild -bl ~/rpmbuild/SPECS/supertuxkart.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
    ...
    sed -i -e '/%files[\s]*$/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/supertuxkart.spec;     sed -i -e '/%files devel[\s]*$/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/supertuxkart.spec
    rpmbuild -ba ~/rpmbuild/SPECS/supertuxkart.spec
fi
