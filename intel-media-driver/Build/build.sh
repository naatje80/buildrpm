...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/intel-media-driver.spec
yum-builddep -y ~/rpmbuild/SPECS/intel-media-driver.spec
rpmbuild -bi ~/rpmbuild/SPECS/intel-media-driver.spec
rpmbuild -bl ~/rpmbuild/SPECS/intel-media-driver.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/intel-media-driver.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/intel-media-driver.spec
rpmbuild -ba ~/rpmbuild/SPECS/intel-media-driver.spec
