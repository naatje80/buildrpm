...
#! /bin/sh
spectool -R -g -f ~/rpmbuild/SPECS/fdk-aac.spec
yum-builddep -y ~/rpmbuild/SPECS/fdk-aac.spec
rpmbuild -bi ~/rpmbuild/SPECS/fdk-aac.spec
rpmbuild -bl ~/rpmbuild/SPECS/fdk-aac.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/fdk-aac.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/fdk-aac.spec
