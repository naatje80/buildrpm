...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/handbrake.spec
yum-builddep -y ~/rpmbuild/SPECS/handbrake.spec
rpmbuild -bi ~/rpmbuild/SPECS/handbrake.spec
if [[ 0 -eq 0 ]]
then
    rpmbuild -bl ~/rpmbuild/SPECS/handbrake.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
    ...
    sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/handbrake.spec;     sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/handbrake.spec
    rpmbuild -ba ~/rpmbuild/SPECS/handbrake.spec
fi
