...
#! /bin/sh
spectool -R -g -f ~/rpmbuild/SPECS/intel-gmmlib.spec
yum-builddep -y ~/rpmbuild/SPECS/intel-gmmlib.spec
rpmbuild -bi ~/rpmbuild/SPECS/intel-gmmlib.spec
rpmbuild -bl ~/rpmbuild/SPECS/intel-gmmlib.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/intel-gmmlib.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/intel-gmmlib.spec
rpmbuild -ba ~/rpmbuild/SPECS/intel-gmmlib.spec
