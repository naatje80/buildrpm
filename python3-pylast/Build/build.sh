...
#! /bin/sh
spectool -R -g -f ~/rpmbuild/SPECS/python3-pylast.spec
yum-builddep -y ~/rpmbuild/SPECS/python3-pylast.spec
rpmbuild -bi ~/rpmbuild/SPECS/python3-pylast.spec
rpmbuild -bl ~/rpmbuild/SPECS/python3-pylast.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/python3-pylast.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/python3-pylast.spec
rpmbuild -ba ~/rpmbuild/SPECS/python3-pylast.spec
