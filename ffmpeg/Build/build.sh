...
#! /bin/sh
spectool -R -g -f ~/rpmbuild/SPECS/ffmpeg.spec
yum-builddep -y ~/rpmbuild/SPECS/ffmpeg.spec
rpmbuild -bi ~/rpmbuild/SPECS/ffmpeg.spec
rpmbuild -bl ~/rpmbuild/SPECS/ffmpeg.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/ffmpeg.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/ffmpeg.spec
