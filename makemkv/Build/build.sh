...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/makemkv.spec
yum-builddep -y ~/rpmbuild/SPECS/makemkv.spec
rpmbuild -bi ~/rpmbuild/SPECS/makemkv.spec
rpmbuild -bl ~/rpmbuild/SPECS/makemkv.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/makemkv.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/makemkv.spec
