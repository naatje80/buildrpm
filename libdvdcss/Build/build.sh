...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/libdvdcss.spec
yum-builddep -y ~/rpmbuild/SPECS/libdvdcss.spec
rpmbuild -bi ~/rpmbuild/SPECS/libdvdcss.spec
rpmbuild -bl ~/rpmbuild/SPECS/libdvdcss.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/libdvdcss.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/libdvdcss.spec
rpmbuild -ba ~/rpmbuild/SPECS/libdvdcss.spec
