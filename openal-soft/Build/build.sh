...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/openal-soft.spec
yum-builddep -y ~/rpmbuild/SPECS/openal-soft.spec
rpmbuild -bi ~/rpmbuild/SPECS/openal-soft.spec
if [[ 0 -eq 0 ]]
then
    rpmbuild -bl ~/rpmbuild/SPECS/openal-soft.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
    ...
    sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/openal-soft.spec;     sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/openal-soft.spec
    rpmbuild -ba ~/rpmbuild/SPECS/openal-soft.spec
fi
