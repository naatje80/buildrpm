...
#! /bin/sh
spectool -R -g -f ~/rpmbuild/SPECS/libx264.spec
yum-builddep -y ~/rpmbuild/SPECS/libx264.spec
rpmbuild -bi ~/rpmbuild/SPECS/libx264.spec
rpmbuild -bl ~/rpmbuild/SPECS/libx264.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/libx264.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/libx264.spec
rpmbuild -ba ~/rpmbuild/SPECS/libx264.spec
