...
#! /bin/sh
spectool -R -g ~/rpmbuild/SPECS/libva-2.7-utils.spec
yum-builddep -y ~/rpmbuild/SPECS/libva-2.7-utils.spec
rpmbuild -bi ~/rpmbuild/SPECS/libva-2.7-utils.spec
rpmbuild -bl ~/rpmbuild/SPECS/libva-2.7-utils.spec 2>&1| sort -u|grep '^   '|grep -v '(but unpackaged)'|tr -d ' ' > /tmp/package_files.log
...
sed -i -e '/%files/r /tmp/FILES.LOG' /root/rpmbuild/SPECS/libva-2.7-utils.spec; sed -i -e '/%files devel/r /tmp/DEVEL_FILES.LOG' /root/rpmbuild/SPECS/libva-2.7-utils.spec
rpmbuild -ba ~/rpmbuild/SPECS/libva-2.7-utils.spec
